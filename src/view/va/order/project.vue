			...
			<el-table :data="list" border class="table top20" ref="multipleTable" @selection-change="handleSelectionChange">
				...
				<el-table-column label="操作" width="430" align="center">
					<template slot-scope="scope">
						...
						<el-button size="mini" @click="sortUp(scope.$index, scope.row)">↑</el-button>
						<el-button size="mini" @click="sortDown(scope.$index, scope.row)">↓</el-button>
	...
	export default {
		...
		methods: {
			...
			sortUp(index, row) {
				...
				orderService.upSort(params).then((res)=>{
					this.$message.success('设置成功');
					t.getItemList();
				})
			},
			...
			sortDown(index, row) {
				...
				orderService.downSort(params).then((res)=>{
					this.$message.success('设置成功');
					t.getItemList();
				})
			},
